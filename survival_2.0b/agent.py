...
class Agent:
    ...
    def _check_surroundings(self, world_map):
        for dx in range(-1, 2):
            for dy in range(-1, 2):
                if dx == 0 and dy == 0:
                    continue
                x, y = self.x + dx, self.y + dy
                resource = world_map.get_resource_at(x, y)
                if resource:
                    if resource.type not in self.resource_memory:
                        self.resource_memory[resource.type] = []
                    if (x, y) not in self.resource_memory[resource.type]:
                        self.resource_memory[resource.type].append((x, y))
                    return ("find_resource", resource.type)
        return None
    ...
    def ai_decide_action(self, world_map):
        # Immediate surroundings check (Priority 1)
        immediate_action = self._check_surroundings(world_map)
        if immediate_action:
    ...
    def execute_action(self, action, world_map):
        ...
        if isinstance(action, tuple):
            ...
            if action_type == "move_to_camp":
                ...
                # Check memory first
                if resource_type in self.resource_memory:
                    for pos in self.resource_memory[resource_type]:
                        dist = abs(pos[0] - self.x) + abs(pos[1] - self.y)
                        if dist < closest_dist:
                            node = world_map.get_resource_at(pos[0], pos[1])
                            if node and not node.depleted:
                                closest = node
                                closest_dist = dist

                # If not in memory, search the whole map
                if not closest:
