...
class Agent:
    ...
    def ai_decide_action(self, world_map):
        ...
        self.decision_weights = {action: self._calculate_action_value(action) for action in self.actions}
    ...
    def _calculate_action_value(self, action):
        score = 0.0

        if action == "eat":
            score = (100 - self.hunger) * 2
        elif action == "drink":
            score = (100 - self.thirst) * 2.5
        elif action == "rest":
